...
public class PublishPackageZipBuilder {
	...
	public void getZip(PublishPackage publishPackage, boolean addZipPrefix, OutputStream os) {
		...
		Optional<List<String>> extensionFilter = getZipExtensionFilter(publishPackage.getPublishConfig());
		...
		for (PublishJob pj: readers.keySet()) {
			...
			writeEntries(zis, zos, folderName, extensionFilter, entries);
	...
	private void writeEntries(ZipInputStream zis, ZipOutputStream zos, String folderName, Optional<List<String>> extensionFilter, HashMap<String, String> entries) {
		String folderPrefix = "";
		...
		try {
			...
			while(nextEntry != null) {
				...
				if (extensionFilter.isPresent() && !entryExt.isBlank() && !extensionFilter.get().contains(entryExt)) {
					logger.info("Skipping entry (by extension filter): {}", nextEntry.getName());
					zis.closeEntry();
